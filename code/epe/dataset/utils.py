...
def read_filelist(path_to_filelist,
                  num_expected_entries_per_row,
                  check_if_exists=True,
                  root_concat=None):
    """ Loads a file with file paths, each line containing multiple file paths separated by a comma
    ...
    with open(path_to_filelist) as file:
        for i, line in enumerate(file):
            ...
            if root_concat is None:
                ps = [Path(p) for p in t[:num_expected_entries_per_row]]
            else:
                ps = [
                    Path(root_concat) / p
                    for p in t[:num_expected_entries_per_row]
                ]
    ...
    if num_skipped > 0:
        logger.warn(
            f'Skipped {num_skipped} entries since at least one file was missing.'
        )
...
def load_crops(path):
    ...
    with open(path) as file:
        ...
        for row in tqdm(reader):
            crops.append((row['path'], int(row['r0']), int(row['r1']),
...
def make_scale_transform(scale):
    return lambda img: rescale(
        img, scale, preserve_range=True, anti_aliasing=True, multichannel=True)


def make_scale_transform_w(target_width):
    return lambda img: rescale(img,
                               float(target_width) / img.shape[1],
                               preserve_range=True,
                               anti_aliasing=True,
                               multichannel=True)
...
def make_scale_transform_h(target_height):
    return lambda img: rescale(img,
                               float(target_height) / img.shape[0],
                               preserve_range=True,
                               anti_aliasing=True,
                               multichannel=True)
